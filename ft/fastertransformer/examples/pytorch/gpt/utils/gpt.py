...
class GPTWeights:
    ...
    def dummy_weights(self, file_path: str, tp_size: int):
        if "175b" in file_path:
            max_seq_len=2048
            ...
            print("model not supported for dummy weights")
        if "input_layernorm.weight" in file_path:
            shape = (hidden_size,)
        elif "input_layernorm.bias" in file_path:
            shape = (hidden_size,)
        elif "attention.query_key_value.weight" in file_path:
            shape = (hidden_size * hidden_size * 3 // tp_size,)
        elif "attention.query_key_value.bias" in file_path:
            shape = (hidden_size * 3 // tp_size,)
        elif "attention.dense.weight" in file_path:
            shape = (hidden_size * hidden_size // tp_size,)
        elif "attention.dense.bias" in file_path:
            shape = (hidden_size,)
        elif "post_attention_layernorm.weight" in file_path:
            shape = (hidden_size,)
        elif "post_attention_layernorm.bias" in file_path:
            shape = (hidden_size,)
        elif "mlp.dense_h_to_4h.weight" in file_path:
            shape = (hidden_size * hidden_size * 4 // tp_size,)
        elif "mlp.dense_h_to_4h.bias" in file_path:
            shape = (hidden_size * 4 // tp_size,)
        elif "mlp.dense_4h_to_h.weight" in file_path:
            shape = (hidden_size * hidden_size * 4 // tp_size,)
        elif "mlp.dense_4h_to_h.bias" in file_path:
            shape = (hidden_size,)
        elif "final_layernorm.weight" in file_path:
            shape = (hidden_size,)
        elif "final_layernorm.bias" in file_path:
            shape = (hidden_size,)
        elif "wpe" in file_path:
            shape = (hidden_size * 2048,)
        elif "wte" in file_path:
            shape = (hidden_size * 50272,)
        elif "lm_head.weight" in file_path:
            shape = (hidden_size * 50272,)
        else:
            raise ValueError(f"Invalid filepath: {file_path}")
        return torch.rand(shape, dtype=str_type_map[self.inference_data_type], device="cuda") / 1e9
    ...
    def load(self, ckpt_path, tp_rank, pipeline_para_rank):
        ...
        def load_to_torch(file_path: str, is_load: bool):
            if is_load:
                # ret = torch.from_numpy(np.fromfile(file_path, dtype=self.weights_data_type)).to(str_type_map[self.inference_data_type])
                # print(file_path, ret.shape)
                # return ret
                return self.dummy_weights(file_path, self.tensor_para_size)
