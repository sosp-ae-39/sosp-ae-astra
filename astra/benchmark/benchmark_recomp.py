...
def main(args: argparse.Namespace):
    ...
    # Benchmark.
    latencies = []
    for _ in tqdm(range(10), desc="Profile step"):
        latencies.append(profile_step(profile=False))

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    output_subdir = os.path.join(
        OUTPUT_DIR, f'recompute-block{args.block_size}.pkl')
    with open(output_subdir, 'wb') as f:
        pickle.dump(np.mean(latencies), f)
